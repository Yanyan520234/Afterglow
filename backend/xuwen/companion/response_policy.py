...
_STORY_PATTERNS = (
    "讲个笑话", "讲笑话", "说个笑话", "来个笑话", "讲个故事", "讲故事",
    "说个故事", "讲一段", "讲段子", "来段", "说段", "讲个",
)
...
def decide_response_policy(
    *,
    ...
    # 注意：带图消息时（has_images=True），VLM 描述里的"图片/表情包"等词
    # 可能误触发下面的要图/要表情模式。图片响应必须优先：真实图片本身
    # 就是要回的"内容"，而不是把用户当成在索图/索表情。
    if _contains_any(text, _IMAGE_PATTERNS) and not has_images:
        mode = "image"
        focus = "ai_continuity"
        use_image = True
        instructions.append("用户有要图倾向；如果系统没有可发送图片能力，不要编造已经发图。")

    if _contains_any(text, _STICKER_PATTERNS) and not has_images:
        mode = "sticker"
        use_sticker = True
        instructions.append("用户有表情包需求；可优先用已配置表情包。")

    if has_images:
        # 带图消息：图片本身就是要回应的内容，模式固定为 image（回应图片）。
        # VLM 描述里的"表情包/图片/照片"等词不得触发 sticker/image 索图模式。
        mode = "image"
        focus = "user_new"
    ...
    if _looks_like_joke(text, lowered) and risk == "low":
        ...
        # 讲笑话/讲故事类请求需要完整展开，不能用极短句；纯接梗才 very_short。
        if _contains_any(text, _STORY_PATTERNS):
            max_length = "medium"
        elif max_length == "very_short":
            max_length = "short"
