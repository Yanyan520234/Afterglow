...
class ProactiveEngine:
    ...
    def _next_schedule_at_ms(
        ...
        *,
        ...
        if profile is None or profile.sample_size <= 0:
            return earliest
        candidate = _next_profile_time_ms(profile, now=now, earliest_ms=earliest)
        # 画像匹配出的"TA 历史活跃时段"可能落在未来安静时段内（例如 23:00 建候选，
        # 画像 01:00 活跃，但 01:00 在 00:00-08:30 安静时段）。到点二次门控虽然会拦截，
        # 但候选会白白到期一轮。这里直接把画像匹配时间与安静时段结束取大，一次排准。
        candidate_quiet_end = _quiet_hours_end(
            datetime.fromtimestamp(candidate / 1000, tz=now.tzinfo),
            self.settings.proactive_quiet_hours,
        )
        if candidate_quiet_end is not None:
            candidate = max(candidate, candidate_quiet_end)
        return candidate
    ...
    def _append_audit_line(self, record: dict[str, Any]) -> None:
        try:
            ...
            logging.getLogger("proactive").warning(
                "poll audit 追加写失败，跳过本条", exc_info=True
            )
    ...
    def _save_poll_state(self) -> None:
        try:
            ...
            # 主动消息状态是可选优化：写盘失败（Windows 下杀软/句柄冲突等）只记日志，
            # 绝不让它把主聊天请求拖成 500。
            logging.getLogger("proactive").warning(
                "poll state 写盘失败，跳过本次保存", exc_info=True
            )
