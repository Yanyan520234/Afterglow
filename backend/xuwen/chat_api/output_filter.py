...
# 需本人处理的现实邀约/请求协议：与 <schedule-hint> 相同的剥离规则。
_NEED_OWNER_RE = re.compile(
    r"\s*(?:<need-owner>.*?(?:</need-owner>|$)|</need-owner>)\s*",
    re.DOTALL | re.IGNORECASE,
)
_NEED_OWNER_OPEN_TAG = "<need-owner>"
_NEED_OWNER_CLOSE_TAG = "</need-owner>"
# 发图协议：主模型输出 <send-image>检索关键词</send-image>，桥接据此从本地图库检索发图。
_SEND_IMAGE_RE = re.compile(
    r"\s*(?:<send-image>.*?(?:</send-image>|$)|</send-image>)\s*",
    re.DOTALL | re.IGNORECASE,
)
_SEND_IMAGE_OPEN_TAG = "<send-image>"
_SEND_IMAGE_CLOSE_TAG = "</send-image>"
...
def sanitize_assistant_text(
    ...
    *,
    ...
    out = _NEED_OWNER_RE.sub("", out)
    out = _SEND_IMAGE_RE.sub("", out)
    ...
    def feed(self, piece: str) -> str:
        ...
        if last_hint_open >= 0:
            ...
            if hint_close_idx == -1 or (
                ...
                cut = last_hint_open
        # 同样保护 <need-owner>...</need-owner> 协议块
        last_owner_open = buffer_lower.rfind(_NEED_OWNER_OPEN_TAG, 0, cut)
        if last_owner_open >= 0:
            owner_close_idx = buffer_lower.find(_NEED_OWNER_CLOSE_TAG, last_owner_open)
            if owner_close_idx == -1 or (
                owner_close_idx + len(_NEED_OWNER_CLOSE_TAG) > cut
            ):
                cut = last_owner_open
        # 同样保护 <send-image>...</send-image> 协议块
        last_img_open = buffer_lower.rfind(_SEND_IMAGE_OPEN_TAG, 0, cut)
        if last_img_open >= 0:
            img_close_idx = buffer_lower.find(_SEND_IMAGE_CLOSE_TAG, last_img_open)
            if img_close_idx == -1 or (
                img_close_idx + len(_SEND_IMAGE_CLOSE_TAG) > cut
            ):
                cut = last_img_open
        if cut <= 0:
            return ""
