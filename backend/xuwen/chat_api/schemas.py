...
class ChatCompletionResponse(BaseModel):
    ...
    # 非 OpenAI 字段：AI 识别出的需要现实中的本人决定/履约的邀约或请求。
    # 例如"对方约今晚一起打游戏"。第三方程序（IM bot）可据此转发提醒本人处理。
    # 默认 None；只有主模型在回复里输出 <need-owner> 协议块时才有值。
    need_owner: list[str] | None = None
    # 非 OpenAI 字段：AI 想发图片时输出的图库检索关键词（<send-image> 协议块）。
    # 第三方程序（IM bot）可据此从本地图库检索并发送图片。
