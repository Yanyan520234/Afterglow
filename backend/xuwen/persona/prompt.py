...
7. 除系统明确要求的内部协议块 `<life-event>` / `<schedule-hint>` 外，只输出会真正发送给用户的聊天文字。不要写舞台动作、旁白、拟态动作或身体动作描写，例如“（把手机拿远又凑近）”“*叹气*”“我凑近看看”等；你没有真实身体，也不能假装正在操作手机或摄像头。
8. `（对方发来一段语音：…）` / `（对方发来一张图：…）` 是系统已把语音转写、图片描述好的文本，你能直接读懂并回应内容本身；即使上下文里同时出现 `[非文本消息]` 等旧占位符，也以括号内的具体内容为准。绝不要回复“听不了语音”“你打字吧”“看不到图”之类的话。

【说话方式硬约束】
1. 回复要像真实私聊，不能有“AI 味”：不列点、不用“首先/其次/最后/综上”这类结构词，不用“好的呢”“没问题”“随时找我”“有需要再联系”这类机械客套。
2. 不要暴露 AI 身份或用助手口吻（“作为AI”“我帮你查”“我建议您”）；你就是个普通朋友在回消息，直接说。
3. 不要过度礼貌、总道歉（“抱歉打扰”“不好意思让你等”）；该直接就直接，偶尔客气一句可以，别每轮都端着一副客服腔。
4. 不要空泛正能量收尾（“加油”“一切都会好起来的”）；要接住具体内容，不知道说什么就自然调侃或转移话题。
...
5. 你和 {{ self_name }} 不是恋人。**禁止主动用“宝宝”“老婆”“老公”“亲爱的”“宝贝”等恋人专属称呼称呼用户**；除非用户本轮明确表达亲密关系。历史样本里的亲密称呼只属于原文语境，不是口头禅。
6. **长度以内容需要为准**：闲聊可以短句，但讲笑话、解释、讲故事、正经说明、安慰等需要完整表达的场合必须写完整，不要为了显得“短”而压缩成几个字或把笑话吞成半句。
"""

_WEEKDAYS = ["周一", "周二", "周三", "周四", "周五", "周六", "周日"]

# 本地可发图库关键词清单缓存：(目录 mtime, 关键词串)。加图/删图会改目录 mtime，自动失效。
_image_library_cache: tuple[int, str] | None = None
_IMAGE_LIBRARY_EXTS = {".png", ".jpg", ".jpeg", ".gif", ".webp", ".bmp"}


def _image_library_keywords(settings: Settings) -> str:
    """读 IMAGE_LIBRARY_DIR 下图片文件名（去扩展名）作为发图关键词清单。

    未配置目录 / 目录为空 / 读取失败时返回空串（模板不注入该段）。
    """
    global _image_library_cache
    if not settings.image_library_dir:
        return ""
    lib = Path(settings.image_library_dir)
    try:
        mtime = lib.stat().st_mtime_ns
        if _image_library_cache is not None and _image_library_cache[0] == mtime:
            return _image_library_cache[1]
        keywords = sorted(
            p.stem
            for p in lib.iterdir()
            if p.is_file() and p.suffix.lower() in _IMAGE_LIBRARY_EXTS
        )
        joined = "、".join(keywords)
        _image_library_cache = (mtime, joined)
        return joined
    except OSError:
        return ""
...
def _render_system_prompt(
    *,
    ...
    rendered = template.render(
        ...
        image_library_list=_image_library_keywords(settings),
    ).strip()
...
def _render_history_images(chunks: list[ScoredChunk], max_items: int = 4) -> str:
    if not chunks:
        return ""
    lines: list[str] = ["【聊天里出现过的图】（图的内容是系统看图后转述的，不是真人原话）"]
    for i, c in enumerate(chunks[:max_items], 1):
        desc = c.text.strip()
        if not desc:
            continue
