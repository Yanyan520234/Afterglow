...
def _night_profile(active_hours: list[int]) -> ProactiveProfile:
    """画像是 TA 在指定小时常主动开聊。"""
    hours = [0.0] * 24
...
@pytest.mark.asyncio
async def test_proactive_poll_skips_quiet_hours_when_profile_peaks_inside(tmp_path):
    """画像活跃小时落在安静时段内时，候选应被排到安静时段结束之后。

    回归：_next_schedule_at_ms 之前只校验"当前是否在安静时段"，不校验画像
    匹配出的未来时间；深夜建候选 + 画像凌晨活跃会把候选排进 00:00-08:30。
    """
    settings = _settings(
        tmp_path,
    ...
    save_proactive_profile(
        _night_profile(active_hours=[1]),
        settings.persona_data_dir / "proactive_profile.json",
    )
    engine = ProactiveEngine(settings)
    now = datetime(2026, 7, 4, 23, 0, tzinfo=UTC)

    result = await engine.poll(conversation_id="conv-1", life=_life(), now=now)
    assert result.state == "scheduled"
    assert result.should_send is False

    scheduled = datetime.fromtimestamp(result.next_poll_at_ms / 1000, tz=UTC)
    # 画像只有凌晨 01:00 活跃，但 01:00 在安静时段内 → 必须推迟到 08:30 之后
    assert scheduled.hour >= 8
    assert not (
        datetime(2026, 7, 5, 0, 0, tzinfo=UTC)
        <= scheduled
        < datetime(2026, 7, 5, 8, 30, tzinfo=UTC)
    )


@pytest.mark.asyncio
async def test_proactive_poll_allows_profile_peak_outside_quiet_hours(tmp_path):
    """画像活跃小时不在安静时段时，候选正常排到该活跃小时。"""
    settings = _settings(
        tmp_path,
        proactive_check_interval_seconds=900,
        proactive_quiet_hours="00:00-08:30",
    ...
    save_proactive_profile(
        _night_profile(active_hours=[22]),
        settings.persona_data_dir / "proactive_profile.json",
    )
    engine = ProactiveEngine(settings)
    now = datetime(2026, 7, 4, 10, 0, tzinfo=UTC)
